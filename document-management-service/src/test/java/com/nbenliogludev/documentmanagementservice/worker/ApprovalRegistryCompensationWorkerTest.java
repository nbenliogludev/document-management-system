...
class ApprovalRegistryCompensationWorkerTest {
        ...
        void processCompensations_ShouldUpdateStatus_WhenCompensationSucceeds() throws Exception {
                ...
                when(compensationService.compensateApprovalRegistryFailure(payload.getDocumentId(), event.getId()))
                                .thenReturn(DocumentApprovalCompensationService.CompensationResult.COMPENSATED);

                worker.processCompensations();

                verify(outboxEventRepository)
                                .save(argThat(saved -> saved.getStatus() == OutboxEventStatus.COMPENSATED));
        }

        @Test
        void processCompensations_ShouldUpdateStatusToFailed_WhenCompensationFails() throws Exception {
                OutboxEvent event = new OutboxEvent();
                event.setId(UUID.randomUUID());
                event.setStatus(OutboxEventStatus.FAILED_PERMANENT);
                event.setPayload("{\"documentId\":\"" + UUID.randomUUID()
                ...
                when(objectMapper.readValue(event.getPayload(), ApprovalRegistryCreatePayload.class))
                                .thenReturn(payload);
                when(compensationService.compensateApprovalRegistryFailure(payload.getDocumentId(), event.getId()))
                                .thenReturn(DocumentApprovalCompensationService.CompensationResult.FAILED);

                worker.processCompensations();

                verify(outboxEventRepository)
                                .save(argThat(saved -> saved.getStatus() == OutboxEventStatus.COMPENSATION_FAILED));
        }
